...
{
    public class SelectComponentsSerializer : IResourceSerializer<SelectComponentsExpression>
    {
        public string Tag => "components";
        ...
        {
            if (!(resource is SelectComponentsExpression specificResource)) return null;
            return Serialize(specificResource);
        }

        public XElement Serialize(SelectComponentsExpression resource)
        {
        ...
        public SelectComponentsExpression Deserialize(XElement element)
        {
            if (element.Name != Tag) return null;

            SelectComponentsExpression resource = new SelectComponentsExpression();
        ...
        {
            return typeof(TResource).IsAssignableFrom(typeof(SelectComponentsExpression));
        }
    }
}