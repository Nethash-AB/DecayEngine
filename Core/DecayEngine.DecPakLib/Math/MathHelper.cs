...
namespace DecayEngine.DecPakLib.Math
{
    public static class MathHelper
    {
        ...
        {
            (float x, float y, float z) = degrees;
            return new Vector3(DegreesToRadians(x), DegreesToRadians(y), DegreesToRadians(z));
        }
        ...
        public static bool IsApproximately(this float x, float y)
        {
        ...
        {
            return IsApproximately(x, 0f);
        }

        public static float Fixed(this float x)
        {
            float y = (float) System.Math.Round(x);
            return x.IsApproximately(y) ? y : x;
        }
    }
}